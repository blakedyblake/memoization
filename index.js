...
// ANSWER: Both for loops (.forEacu() is a for loop) loops through the letters of the string
...
class characterList {
  ...
  calcString(string){
    if(this.cache[string]){
      ...
      let arr = string.split(' ').join("").split('').map(e => e.toLowerCase())
      let sum = 0
      for(let i = 0; i <arr.length; i ++){
        if(list[arr[i]]) sum += list[arr[i]]
      }
      this.cache[string] = sum
      return sum
...
// CODE HERE 
function NonCacheCalcString(string){
      let arr = string.split(' ').join("").split('').map(e => e.toLowerCase())
      let sum = 0
      for(let i = 0; i <arr.length; i ++){
        if(list[arr[i]]) sum += list[arr[i]]
...
let charsCalc = new characterList();
console.time("not memoized function time");
console.log(NonCacheCalcString("Hey"))
console.log(NonCacheCalcString("Hey"))
console.log(NonCacheCalcString("Hey"))
console.log(NonCacheCalcString("Hey"))
...
console.log(charsCalc.calcString("Hey"))
console.log(charsCalc.calcString("Hey"))
console.log(charsCalc.calcString("Hey"))
console.log(charsCalc.calcString("Hey"))
